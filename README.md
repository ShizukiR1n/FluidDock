...
「更新 → 检查更新」一行走完整个流程：点一下去 GitHub 看最新 Release，有新版本就变成「更新到 V0.7」，再点一下下载、原地替换、自己重启。**`config\` 一个字节都不碰**，所以 dock.json 和图标列表原样留着 —— 这是它存在的理由：以前装一个修复要重新下载、解压到新目录、把每个图标再加一遍。
